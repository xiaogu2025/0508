  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::exploration_manager::HGrid_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.points1.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::geometry_msgs::Point_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.points1[i]);
    }
    if (v.points1.empty() || false)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.points2.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::geometry_msgs::Point_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.points2[i]);
    }
    if (v.points2.empty() || false)
      s << "]";
  }
