...
void FameExplorationManager::initialize(ros::NodeHandle& nh) {
  ...
  // 新增
  nh.param("method/use_ft_score", use_ft_score_, false);
  nh.param("method/use_underwater_role", use_underwater_role_, false);
  nh.param("method/use_hypergraph_coord", use_hypergraph_coord_, false);

  nh.param("method/w_ft_score", w_ft_score_, 1.0);
  nh.param("method/w_high_order", w_high_order_, 1.0);

  nh.param("method/log_cost_terms", log_cost_terms_, true);
  ...
  underwater_role_selector_.reset(new UnderwaterRoleSelector);
  underwater_role_selector_->init(nh);

  hypergraph_coordinator_.reset(new HypergraphCoordinator);
  hypergraph_coordinator_->init(nh);
...
int FameExplorationManager::planExploreMotion(
  ...
  // 新增
  resetEvalStepDebug();
  eval_role_before_ = role_;

  if (use_path_regularizer_ && path_regularizer_) {
  path_regularizer_->updateHistory(pos);
  }
  ...
  */
  ...
  ROLE updated_role = role_assigner_->assignRole(
    pos, ep_->drone_id_, ed_->swarm_state_, frontier_finder_->getFrontiers());
 
  double s_explorer = 0.0;
  double s_collector = 0.0;

  if (use_underwater_role_) {
    updated_role = underwater_role_selector_->refineRole(
        updated_role,
        pos,
        ep_->drone_id_,
        ed_->swarm_state_,
        frontier_finder_->getFrontiers(),
        &s_explorer,
        &s_collector);

    if (log_cost_terms_) {
      ROS_INFO("[UnderwaterRole] drone=%d S_exp=%.3f S_col=%.3f role=%d",
              ep_->drone_id_, s_explorer, s_collector, int(updated_role));
    }
  }

  updateRoleAndVelocities(updated_role);
  eval_role_after_ = updated_role;

  if (use_hypergraph_coord_) {
    hypergraph_coordinator_->updateContext(
        ep_->drone_id_, ed_->swarm_state_, frontier_finder_->getFrontiers());
  }
  ...
  */
  ...
  if (planTrajToView(pos, vel, acc, yaw, next_pos, next_yaw) == FAIL) {
    traj_success = false;
    logEvalStep(pos, next_pos, next_yaw, false);
    return FAIL;
  }
  ...
  logEvalStep(pos, next_pos, next_yaw, true);     //            新增
  return SUCCEED;
}
...
bool FameExplorationManager::explorerPlan(const Vector3d& pos, const Vector3d& vel,
  ...
  for (const auto& ftr : ed_->infront_frontiers_) {
    ...
    if (use_ft_score_) {
      ft_cost = w_ft_score_ *
          ft_evaluator_->targetBiasFromCells(ftr.first, ftr.second, role_);
      total_cost += ft_cost;
    }
    ...
    if (log_cost_terms_) {
      ROS_INFO("[ExplorerCost] drone=%d base+old=%.3f ft=%.3f ho=%.3f final=%.3f",
              ep_->drone_id_,
              total_cost - ft_cost - high_order_cost,
              ft_cost,
              high_order_cost,
              total_cost);
    }
...
bool FameExplorationManager::greedyPlan(const Vector3d& pos, const Vector3d& vel,
  ...
  for (size_t i = 0; i < vp_pos.size(); ++i) {
    ...
    */
    ...
    if (use_ft_score_) {
      ft_cost = w_ft_score_ *
          ft_evaluator_->targetBiasSimple(vp_label, vp_position, pos, role_);
    }
    ...
    if (total_cost < min_cost) {
      ...
      // 新增
      eval_plan_source_ = "greedy";
      eval_selected_label_ = vp_label;
...
bool FameExplorationManager::findTourOfTrails(const Vector3d& cur_pos,
  ...
  */
  ...
  eval_plan_source_ = "trail_tour";            //新增
  eval_selected_label_ = LABEL::TRAIL;         //新增
...
// ----------------------新增
void FameExplorationManager::initEvalLogger(ros::NodeHandle& nh) {
  nh.param("eval/enable", eval_enable_, false);
  nh.param("eval/dir", eval_dir_, std::string("/tmp/fame_eval"));
  nh.param("eval/exp_name", eval_exp_name_, std::string("A0"));
  nh.param("eval/run_name", eval_run_name_, std::string("run1"));
  ...
  if (!eval_file_.is_open()) {
    ROS_ERROR_STREAM("[Eval] Failed to open eval file: " << file_path);
    eval_enable_ = false;
    return;
  }
  ...
  ROS_WARN_STREAM("[Eval] Logging to: " << file_path);
}

void FameExplorationManager::closeEvalLogger() {
  if (eval_file_.is_open()) {
    eval_file_.close();
  }
}

std::string FameExplorationManager::roleToString(const ROLE& r) const {
  if (r == ROLE::EXPLORER) return "EXPLORER";
  if (r == ROLE::GARBAGE_COLLECTOR) return "COLLECTOR";
  if (r == ROLE::UNKNOWN) return "UNKNOWN";
  return "OTHER";
}

std::string FameExplorationManager::labelToString(const LABEL& l) const {
  if (l == LABEL::FRONTIER) return "FRONTIER";
  if (l == LABEL::TRAIL) return "TRAIL";
  if (l == LABEL::UNLABELED) return "UNLABELED";
  return "OTHER";
}

void FameExplorationManager::resetEvalStepDebug() {
  eval_selected_label_ = LABEL::UNLABELED;
  eval_plan_source_ = "none";

  eval_base_cost_ = 0.0;
  eval_ft_cost_ = 0.0;
  eval_high_order_cost_ = 0.0;

  eval_s_explorer_ = 0.0;
  eval_s_collector_ = 0.0;

  eval_duplicate_target_count_ = 0;
  eval_comm_risk_count_ = 0;
...
void FameExplorationManager::countFrontierLabels(
    int& n_frontier, int& n_trail, int& n_unlabeled) const {
  n_frontier = 0;
  n_trail = 0;
  n_unlabeled = 0;

  const auto& frontiers = frontier_finder_->getFrontiers();
...
void FameExplorationManager::logEvalStep(
    const Eigen::Vector3d& cur_pos,
    const Eigen::Vector3d& next_pos,
    double next_yaw,
    bool success) {
  if (!eval_enable_ || !eval_file_.is_open()) return;

  double now = ros::Time::now().toSec();

  if (eval_start_time_ < 0.0) {
    eval_start_time_ = now;
  }

  double rel_time = now - eval_start_time_;
  eval_last_time_ = rel_time;

  if (!eval_has_last_pos_) {
    eval_last_pos_ = cur_pos;
    eval_has_last_pos_ = true;
  } else {
    eval_travel_dist_ += (cur_pos - eval_last_pos_).norm();
    eval_last_pos_ = cur_pos;
  }
  ...
  int n_trail = 0;
  int n_unlabeled = 0;
  countFrontierLabels(n_frontier, n_trail, n_unlabeled);
  ...
  for (int i = 0; i < static_cast<int>(ed_->swarm_state_.size()); ++i) {
    ...
    double d_comm = (ed_->swarm_state_[i].pos_ - next_pos).norm();
    if (d_comm > eval_comm_range_) {
      ++comm_risk_count;
    }
  }

  eval_duplicate_target_count_ = duplicate_count;
  eval_comm_risk_count_ = comm_risk_count;
  ...
  if (use_hypergraph_coord_ && hypergraph_coordinator_) {
    j_comp = hypergraph_coordinator_->lastCompetitionCost();
    j_comm = hypergraph_coordinator_->lastCommCost();
    j_red = hypergraph_coordinator_->lastRedundantCleanupCost();
  }
  ...
  eval_file_
      ...
      << eval_plan_step_++ << ","
      << ep_->drone_id_ << ","
      << roleToString(eval_role_before_) << ","
      << roleToString(eval_role_after_) << ","
      << eval_plan_source_ << ","
      << labelToString(eval_selected_label_) << ","
